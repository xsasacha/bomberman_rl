...
BOMBED_CRATE_EVENT = "BOMBED_CRATE"
NOT_BOMB_CRATE_EVENT = "NOT_BOMB_CRATE"

NO_PROGRESS_EVENT = "NO_PROGRESS"
STEP_POSSIBLE_EVENT = "STEP_POSSIBLE"
IMPOSSIBLE_STEP_EVENT = "IMPOSSIBLE_STEP"
...
def game_events_occurred(self, old_game_state: dict, self_action: str, new_game_state: dict, events: List[str]):
    ...
    if old_game_state:
        ...
        coin_dir = np.asarray([old_features[0], old_features[1]]) #Ftr 1&2
        crate_dir = np.asarray([old_features[6], old_features[7]]) #Ftr 7&8
        safety_dir = np.asarray([old_features[8], old_features[9]]) #Ftr 9&10
        offense_dir = np.asarray([old_features[11], old_features[12]]) #Ftr 12&13
        ...
        elif not check_dir(coin_dir, self_action):
            if not(old_player_pos == new_player_pos):
     ...
     #---------------------------------------------------
        ...
        if self_action == "LEFT" or self_action == "RIGHT" or self_action == "UP" or self_action == "DOWN":
            if not check_action(self_action, old_features):
                events.append(IMPOSSIBLE_STEP_EVENT)
        else:
     ...
     #---------------------------------------------------
        ...
        elif not(check_dir(crate_dir, self_action)):
           if not(old_player_pos == new_player_pos):
     ...
     #----------------------------------------------------
        ...
        if old_features[13] == 1 and self_action == 'BOMB':
            events.append(BOMBED_CRATE_EVENT)
        elif old_features[13] == 1 and self_action != 'BOMB':
            events.append(NOT_BOMB_CRATE_EVENT)
...
def reward_from_events(self, events: List[str]) -> int:
    ...
    *This is not a required function, but an idea to structure your code.*
    ...
    game_rewards = {
        ...
        CLOSER_TO_ENEMY_EVENT: 2,
        REMOVED_FROM_ENEMY_EVENT: -1,

        DEFENSIVE_STEP_EVENT: 5,
        IN_BLAST_RANGE_EVENT: -5,

        BOMBED_CRATE_EVENT: 3,
        NOT_BOMB_CRATE_EVENT: -2,
