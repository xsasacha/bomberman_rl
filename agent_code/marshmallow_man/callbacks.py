...
def find_fields_of_type(start_coordinates, arena, field_type = 0):
    ...
    while not q.empty():
        ...
        # do all possible steps for a position (UP, DOWN, LEFT, RIGHT) -> since the arena is surrounded by walls we don't need to check if a step would exceed the boundaries of the arena error, every path stops beforeep
        steps = [[-1, 0], [1, 0], [0, -1], [0, 1]]
        for step in steps:
            new_position = current_element + np.asarray(step)
...
def maximum_crates(game_state, max_range):
    ...
    #find best position to bomb in the reachable positions in a given range
    max_crates = 0
    max_direction = [0, 0]
...
def state_to_features(game_state: dict) -> np.array:
    ...
    *This is not a required function, but an idea to structure your code.*
    ...
    for coin in coins:
        distance = np.linalg.norm(np.asarray(player) - np.asarray(coin))
    ...
    #--------------------------------------
    #Feature 6 - 9 check if movement in the direction is possible (order: UP, DOWN, LEFT, RIGHT) -> wasn't helpful, so we dropped it
    #--------------------------------------
    ...
    # End of coin features ----------------------------------------------------------------------------------------------------------------------------- start of crate features
    ...
    # #------------------------------------
    # #Feature 11: direction to position with max number of destructable crates
    # #--------------------------------------
    n = 20
    ...
    # End of crate features ----------------------------------------------------------------------------------------------------------------------------- start of fight features
